-- Issue #44 correction, 9 Sep 2026.
-- Broker licensing/authorization is maintained by OJK. PertaLife must not
-- maintain or infer an Active/Inactive business status for Broker records.
-- Existing master rows are preserved exactly; this migration updates schema
-- semantics and the add-only Broker bulk RPC only.

alter table public.master_brokers
  alter column status drop not null,
  alter column status drop default;

comment on column public.master_brokers.status is
  'Legacy compatibility field only. Not a PertaLife-maintained Broker status and not an OJK licensing/authorization determination. New Broker workflows must not write, display, filter, or gate on this field.';

-- Remove the PR #46 signature that required a PertaLife-chosen Broker status.
drop function if exists public.bulk_add_master_brokers(jsonb, text, text, text);
...
begin
  ...
  -- Full preflight before any INSERT keeps the transaction atomic.
  ...
  loop
    ...
    insert into public.master_brokers (
      ...
      v_phone1, v_phone2, v_fax, v_email, v_website, null,
...
revoke all on function public.bulk_add_master_brokers(jsonb, text, text) from public;
grant execute on function public.bulk_add_master_brokers(jsonb, text, text) to authenticated;
