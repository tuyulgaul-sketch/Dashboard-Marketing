    ...
    <AppLayout>
      <div className="space-y-5">
        ...
        <Tabs value={tab} onValueChange={value => setTab(value as UploadTab)} className="w-full">
          <TabsList className="grid h-auto w-full grid-cols-1 gap-1 bg-white p-1 sm:grid-cols-4">
        ...
        <div className="flex items-start gap-2 rounded-lg border border-blue-100 bg-blue-50/50 p-3 text-xs leading-relaxed text-blue-900"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><p>Upload dan penghapusan hanya tersedia bagi publisher resmi. Setiap penghapusan memerlukan preview, alasan, dan konfirmasi ID batch. Snapshot lengkap disimpan untuk audit; data lain tidak direset.</p></div>
      </div>
    </AppLayout>
  );
};
