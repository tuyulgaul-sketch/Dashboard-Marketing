...
const loadInitialTab = (): UploadTab => {
  try {
    ...
    return stored && VALID_TABS.has(stored) ? stored : 'targets';
  } catch {
    return 'targets';
  }
};
    ...
    <AppLayout>
      <div className="space-y-5">
        <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div><h1 className="text-xl font-bold text-slate-900">Setup Target dan Realisasi</h1><p className="mt-1 text-xs text-slate-500">Pengelolaan Target RKAP melalui Excel, Bulk Pipeline, dan Realisasi Produksi Official.</p><p className="mt-2 text-[11px] font-semibold text-blue-700">Otoritas: Arianie Fajarwati • Marketing Support</p></div>
        </div>
        <Tabs value={tab} onValueChange={value => setTab(value as UploadTab)} className="w-full">
          <TabsList className="grid h-auto w-full grid-cols-1 gap-1 bg-white p-1 sm:grid-cols-2 lg:grid-cols-4">
