...
import {
  ...
  type MarketingToolBrowseCategory,
  type MarketingToolBrowseSelection,
} from '@/lib/marketingToolBrowse';
...
/** Read-only catalogue. The existing Marcomm publisher and approval workflows are separate. */
export const MarketingToolBrowser: React.FC<Props> = ({ documents, products, onDownload, loading = false }) => {
  const [selection, setSelection] = useState<MarketingToolBrowseSelection>({ product: 'ALL', category: null, search: '' });
  const [downloadingId, setDownloadingId] = useState<string | null>(null);
  const productOptions = useMemo(() => getMarketingToolProductOptions(products, documents), [products, documents]);
  const view = useMemo(() => getMarketingToolBrowseView(documents, selection), [documents, selection]);
  ...
  const changeProduct = (product: string) => {
    if (loading || downloadingId) return;
    if (product !== 'ALL' && !productOptions.includes(product)) return;
    setSelection({ product, category: null, search: '' });
  };

  const changeCategory = (category: MarketingToolBrowseCategory) => {
  ...
  const handleDownload = async (document: ManagedServiceDocument) => {
    if (loading || downloadingId || !productOptions.includes(selection.product) ||
      !canDownloadMarketingTool(documents, selection, document)) return;
  ...
  const selectedCategoryLabel = MARKETING_TOOL_BROWSE_CATEGORIES.find(item => item.value === view.category)?.label || '';

  return (
    <div className="space-y-5">
      <div className="space-y-2">
        <label className="block text-xs font-bold text-gray-700" id="marketing-tool-product-label">1. Pilih produk</label>
        <AdminProductPicker value={selection.product} options={productOptions} onChange={changeProduct}
          disabled={busy} labelId="marketing-tool-product-label" />
        <p className="text-xs text-gray-500">Cari dan pilih produk yang sesuai sebelum membuka marketing tools. Pilihan Semua Produk hanya menampilkan jumlah dokumen.</p>
      </div>

      <div className="space-y-2">
        <div className="text-xs font-bold text-gray-700">2. Pilih jenis marketing tool</div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {MARKETING_TOOL_BROWSE_CATEGORIES.map(category => {
      ...
      {loading ? (
        <p className="text-sm text-gray-500" role="status">Memuat marketing tools...</p>
        ...
        <div className="rounded-xl border border-dashed border-gray-200 p-6 text-center text-sm text-gray-500" role="status">
          Pilih produk terlebih dahulu untuk mengaktifkan kartu Proposal, Materi Presentasi, atau Brosur.
        </div>
      ) : !view.category ? (
        <div className="rounded-xl border border-dashed border-gray-200 p-6 text-center text-sm text-gray-500" role="status">
          Produk <span className="font-semibold text-gray-700">{selection.product}</span> dipilih. Klik salah satu kartu untuk melihat marketing tools yang tersedia.
        </div>
      ) : (
        <section className="space-y-3" aria-label={`${selectedCategoryLabel} untuk ${selection.product}`}>
          ...
          <div className="relative">
            ...
            <Input value={selection.search} onChange={event => setSelection(previous => ({ ...previous, search: event.target.value }))}
              placeholder="Cari judul, nama file, atau versi..." aria-label="Cari marketing tool pada produk dan jenis terpilih" className="pl-9 text-xs" />
          </div>
              ...
              <CardContent className="flex flex-wrap items-center justify-between gap-4 p-4">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                  ...
                  <div className="min-w-0 space-y-1">
                    ...
                    <div className="text-xs text-gray-500">{selectedCategoryLabel} • {document.productName} • {document.versionLabel || `V${document.version}`}</div>
                ...
                <Button type="button" size="sm" variant="outline" disabled={busy || !canDownloadMarketingTool(documents, selection, document)} onClick={() => void handleDownload(document)}>
          ...
          {view.visibleDocuments.length === 0 && (
            <div className="rounded-xl border border-dashed border-gray-200 p-8 text-center text-sm text-gray-500">
              {selection.search.trim() ? 'Tidak ada dokumen yang sesuai pencarian pada produk dan jenis ini.' : `Belum ada ${selectedCategoryLabel} published untuk produk ${selection.product}.`}
            </div>
          )}
        </section>
      )}
...
export default MarketingToolBrowser;
