...
const OfficialUploadArchiveManager: React.FC = () => {
  ...
  const authorized = canPublish && !loading;
  ...
  const archive = async () => {
    ...
    try {
      ...
      try {
        ...
        setMessage(`Data diarsipkan. Bukti: ${receipt.archiveId}. ${receipt.recordCount} record dikeluarkan dari data aktif.`);
      } catch (refreshError) {
        setRows(current => current.filter(row => keyOf(row) !== keyOf(receipt)));
        setFailure(true);
        setMessage(`Arsip berhasil tersimpan (${receipt.archiveId}), tetapi penyegaran gagal: ${errorText(refreshError)}. Muat ulang halaman; jangan ulangi penghapusan.`);
      }
    } catch (error) { setSelected(null); setFailure(true); setMessage(errorText(error)); }
    finally { setBusy(false); }
  };
  return <Card className="border-slate-200">
    <CardHeader><CardTitle className="flex items-center gap-2 text-base"><Archive className="h-5 w-5 text-slate-700"/>Kelola / Hapus Data Upload</CardTitle><CardDescription>Penghapusan berdasarkan batch atau ID sumber yang pasti. Snapshot lengkap disimpan di arsip database untuk pemulihan oleh IT. Tidak ada tombol hapus seluruh database.</CardDescription></CardHeader>
    <CardContent className="space-y-4">
      ...
      {authorized && <>
        ...
        {selected && <div className="space-y-4 rounded-xl border border-red-200 bg-red-50/40 p-4">
          <div><h3 className="text-sm font-semibold">Konfirmasi penghapusan {labels[selected.kind]}</h3><p className="mt-1 break-all text-xs text-slate-600">{selected.id}</p></div>
