...
const TargetCascadingWizard: React.FC<{ publisherAuthorized: boolean; active?: boolean }> = ({ publisherAuthorized, active = true }) => {
  ...
  workspaceRef.current = { key: draftKey, shouldSave: authorized && loaded && loadedYear === year && (dirty || modalOpen), saved: { version: 3, year, actorId, sourceSnapshot, sourceDirectory, state: draft, form, modalOpen, reviewMonth, batchNotes, savedAt: new Date().toISOString() } };
  ...
  const loadYear = useCallback(async (selectedYear: number) => {
    setBusy(true); setLoaded(false); setLoadedYear(null); setModalOpen(false); setReview(null); setAcknowledged(false); setMessage(''); setSavedDraft(null);
    try {
  ...
  useEffect(() => { persist(); }, [persist, authorized, loaded, dirty, year, loadedYear, actorId, sourceSnapshot, sourceDirectory, draft, form, modalOpen, reviewMonth, batchNotes]);
  useEffect(() => {
    const save = () => persist();
  ...
  const resume = () => { if (!savedDraft || !tree) return; try { const restored = continueWizard(tree, savedDraft.state); setDraft(restored); setDirty(true); setSavedDraft(null); setModalOpen(true); if (validForm(savedDraft.form)) setForm(savedDraft.form); else clearForm(); persist(true); } catch (error) { setMessage(errorText(error)); } };
  const start = () => { setSavedDraft(null); setModalOpen(true); persist(true); };
  const changeYear = (value: number) => { if (value === year || busy) return; if (dirty && !window.confirm('Draft tetap tersimpan di sesi browser. Ganti tahun?')) return; persist(); setLoaded(false); setLoadedYear(null); setYear(value); };
  const next = () => {
