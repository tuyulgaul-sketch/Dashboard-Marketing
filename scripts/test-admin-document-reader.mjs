...
test('empty catalogue keeps active product choices and zero counts', () => {
  ...
  assert.deepEqual(getPublishedAdminDocumentCounts([]), { SPAJ: 0, SPAK: 0 });
});
...
test('only published SPAJ/SPAK belonging to Marketing Administration are visible', () => {
  const documents = [doc(), doc({ id: '2', category: 'SPAK' }), doc({ id: '3', status: 'DRAFT' }),
    ...
    doc({ id: '6', status: 'PENDING_APPROVAL' })];
  assert.deepEqual(filterPublishedAdminDocuments(documents, all).map(item => item.id), ['doc-1', '2']);
});
...
test('summary counts include all published products and filtering never mutates input', () => {
  ...
  assert.deepEqual(getPublishedAdminDocumentCounts(documents), { SPAJ: 1, SPAK: 1 });
  assert.deepEqual(filterPublishedAdminDocuments(documents, all).map(item => item.id), ['2', '1']);
