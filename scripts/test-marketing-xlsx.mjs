...
const masterSheet = masterBook.addWorksheet('RINCIAN PREMI');
masterSheet.addRow(['No.', 'Bulan Produksi', 'NO POLIS', 'NAMA PEMEGANG POLIS', 'PERUSAHAAN', 'NO. NOTA', 'PRODUK', 'GROSS PREMI', 'DISTRIBUSI PEMASARAN', 'STATUS PREMI', 'USERID', 'USERNAME']);
masterSheet.addRow([1, '2026/001', 'POL-MASTER-001', 'Pemegang Polis', 'PT Master', '', 'PA MEDICARD', 123456.78, 'CORPORATE & RETAIL MARKETING', 'NEW BUSINESS', 'USR-000026', 'Marketing B']);
const masterBytes = new Uint8Array(await masterBook.xlsx.writeBuffer());
const masterImported = await workbook.readMarketingSpreadsheet(file('master-realisasi.xlsx', masterBytes), { sheetName: workbook.MARKETING_SHEETS.production, requiredHeaders: workbook.getMarketingTemplateHeaders('production') });
assert.equal(masterImported.length, 1);
assert.equal(masterImported[0]['Tahun Produksi'], '2026');
assert.equal(masterImported[0]['Bulan Produksi'], '1');
assert.equal(masterImported[0]['Nomor Polis'], 'POL-MASTER-001');
assert.equal(masterImported[0]['Nama Nasabah'], 'PT Master');
assert.equal(masterImported[0]['Realisasi Produksi (Rp)'], '123456.78');
assert.equal(masterImported[0]['User ID Pemilik Realisasi'], 'USR-000026');
assert.equal(masterImported[0]['PIC Marketing'], 'Marketing B');
